...
async function startAndroid(options) {
    ...
    // 所有必要的权限已获得，直接开始语音识别
    return await RNRetuAsr.start(options);
}
...
const RetuAsr = {
     ...
     */
    start(options) {
        return Platform.OS === 'ios' ? RNRetuAsr.start(options) :  startAndroid(options);
    },
     ...
     */
    stop() {
        return RNRetuAsr.stop();
    },
};

export default RetuAsr